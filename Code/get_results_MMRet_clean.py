...
parser.add_argument('--model-name-or-path', default='JUNJIE99/MMRet-large',
...
class RetrievalModel(DRESModel):
    ...
    def __init__(self, **kwargs):
        # Removed token=... and any external links
        self.encoder = AutoModel.from_pretrained(args.model_name_or_path, trust_remote_code=True)
        # self.tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path,trust_remote_code=True,  cache_dir='../cache_dir')
        self.encoder.set_processor(args.model_name_or_path)
    ...
    def encode_queries(self, queries: List[str], **kwargs) -> np.ndarray:
        ...
        input_images = [q['img_path'] for q in queries]
        # else:
    ...
    def encode_corpus(self, corpus: List[Dict[str, str]], **kwargs) -> np.ndarray:
        ...
        input_images = [doc['img_path'] for doc in corpus]  # Only for IT2I
        # # no need to add prefix for instruct models
        # if args.prefix_type == 'query_or_passage':
        #     input_texts = ['passage: {}'.format(t) for t in input_texts]

        return self._do_encode(input_texts, input_images, max_length=4096, is_query=True)
        ...
        for start_idx in tqdm.tqdm(range(0, len(input_texts), batch_size), desc='encoding', mininterval=10):
            ...
            with torch.cuda.amp.autocast():
                ...
                if is_query is True:
                    ...
                    embeds = self.encoder.encode(images=images, text=text)
                else:
                    embeds = self.encoder.encode(text=text)
...
def main():
    ...
    for name in folders:
        ...
        results_dict = evaluation.run(
            model=model,
            output_folder=f'./results_new/IT2I/mm_Ret/{name}',
            eval_splits=["test"],
