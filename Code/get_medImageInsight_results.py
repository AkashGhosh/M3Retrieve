...
#     model_dir="./MedImageInsight/2024.09.27",
...
# # Read image
# image = base64.encodebytes(read_image("../proxy.jpg")).decode("utf-8")
...
parser.add_argument('--model-name-or-path', default='./MedImageInsight/2024.09.27',
...
class RetrievalModel(DRESModel):
    ...
    def encode_corpus(self, corpus: List[Dict[str, str]], **kwargs) -> np.ndarray:
        ...
        # input_images=[q['img'] for q in corpus]
        # # no need to add prefix for instruct models
        # if args.prefix_type == 'query_or_passage':
        #     input_texts = ['passage: {}'.format(t) for t in input_texts]

        return self._do_encode(input_texts, None ,max_length=256,is_query=False)
    ...
    def _do_encode(self, input_texts: List[dict],input_images,max_length:int, is_query:bool) -> np.ndarray:
        ...
        for start_idx in tqdm.tqdm(range(0, len(input_texts), batch_size), desc='encoding', mininterval=10):
            ...
            with torch.cuda.amp.autocast():
                ...
                # embeds=self.encoder.encode(batch_input_texts,instruction='', max_length=32768)
                # text=' '.join(batch_input_texts[0].split(' ')[:50])
                if is_query==True:
                    
                    # print(f"Image :{images}, Text :{text}")
                    individual_embeds=self.encoder.encode(images=batch_input_images, texts=batch_input_texts)
                    embeds=individual_embeds['text_embeddings']+individual_embeds['image_embeddings']
                else:
                    embeds=self.encoder.encode(texts=batch_input_texts)['text_embeddings']
...
# ]
    ...
    for name in folders:
        ...
        results_dict = evaluation.run(model=model,output_folder=f'./results_new/MultiCare/MedImageInsight/{name}',eval_splits=["test"],overwrite_results=True,corpus_chunk_size=400000)
